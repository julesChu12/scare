...
func TestRequestRepository_ListDispatchedByStation(t *testing.T) {
	db := setupTestDB(t)
	repo := NewRequestRepository(db)
	...
	for i := 0; i < 3; i++ {
		req := &model.ServiceRequest{
			RequestNo:   fmt.Sprintf("REQ-D-%d-%d", time.Now().UnixNano(), i),
			UserID:      1,
			ServiceType: consts.ServiceTypeMeal,
	...
	// 创建其他状态的需求
	req := &model.ServiceRequest{
		RequestNo:   fmt.Sprintf("REQ-P-%d", time.Now().UnixNano()),
		UserID:      1,
		ServiceType: consts.ServiceTypeMeal,
		Status:      consts.RequestStatusPending,
		StationID:   1,
	...
	// 查询已分派的需求
	reqs, total, err := repo.ListDispatchedByStation(1, 0, 10)
	if err != nil {
		t.Fatalf("failed to list dispatched requests: %v", err)
	}

	if total != 3 {
		t.Errorf("expected 3 dispatched requests, got %d", total)
	}
...
func TestRequestRepository_UpdateStatus(t *testing.T) {
	...
	// 更新状态
	err := repo.UpdateStatus(req.ID, consts.RequestStatusDispatched)
	if err != nil {
	...
	if updated.Status != consts.RequestStatusDispatched {
		t.Errorf("expected status %s, got %s", consts.RequestStatusDispatched, updated.Status)
	}
}

func TestRequestRepository_UpdateStationAndStatus(t *testing.T) {
	db := setupTestDB(t)
	repo := NewRequestRepository(db)
	...
	// 更新站点和状态
	err := repo.UpdateStationAndStatus(req.ID, 5, consts.RequestStatusDispatched)
	if err != nil {
		t.Fatalf("failed to update station and status: %v", err)
	}

	// 验证更新
	updated, _ := repo.GetByID(req.ID)
	if updated.StationID != 5 {
		t.Errorf("expected station_id 5, got %d", updated.StationID)
	}
	if updated.Status != consts.RequestStatusDispatched {
		t.Errorf("expected status %s, got %s", consts.RequestStatusDispatched, updated.Status)
	}

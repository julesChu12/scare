...
| 模块 | 完成度 | 状态 | 说明 |
|------|--------|------|------|
| **后端核心功能** | 100% | ✅ 完成 | 已通过代码逐模块验证，认证/围栏/需求任务/权限/通知等全部实现 |
...
- ✅ 最终站点匹配统一收敛到“服务地址”
- ✅ `submit_lat/lng` 仅保留为提交位置记录，不再参与最终派单
- ✅ `source_station_id` 仅保留为扫码入口来源记录，不再作为静默兜底派单依据
- ✅ 首页“附近服务站”继续保留，但仅用于展示，不代表最终受理站点

### 地址校验与人工复核

- ✅ C 端创建需求、快速开通统一要求提供服务地址
- ✅ 后端统一通过服务地址 geocode 后再执行围栏匹配 / 最近站点匹配
- ✅ 服务地址解析失败时，不再自动派给来源站点，而是创建 `pending` 状态的人工复核需求
- ✅ 人工复核需求会记录 `dispatch_basis=service_address_manual_review`、`needs_manual_verify=true`，且不创建任务

### C 端申请页调整

- ✅ 移除“服务地点与当前位置一致”开关，避免家属异地下单误派单
- ✅ 页面文案明确为“当前位置仅用于记录提交位置，不参与最终受理站点匹配”
- ✅ 提交前仍可按服务地址预览预计受理站点；若地址无法解析，则明确提示进入人工复核

### 验证结果

- ✅ 后端测试通过：`go test ./internal/service/... ./internal/handler/...`
- ✅ C 端构建通过：`npm run build`
